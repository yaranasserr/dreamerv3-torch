...
def compute_validation_loss(wm, batch):
    """Compute proper validation loss including reconstruction loss"""
    try:
        with torch.no_grad():
            # Get the full model prediction (this is what the original _train method does)
            data = wm.preprocess(batch)
            
            # Encode observations
            embed = wm.encoder(data)
            
            # Get posterior and prior from dynamics
...
def get_hdf5_files_from_pattern(pattern):
    """Get HDF5 files from glob pattern or single file"""
